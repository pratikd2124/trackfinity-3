...
function ViewTest() {
    ...
    const url = "http://13.113.26.29:5000"
    
    const authorid = JSON.parse(localStorage.getItem("trackfinity-user")).data._id
    useEffect(() => {
    ...
    function handlenavigate(id,title) {
        navigate("/teachdashboard/addtest", { state: {testid:id,title:title} });
    }
    ...
    return (
        <Container >
            ...
            {role === "teacher" && <div className='d-flex'><Button className="mt-2 me-2" onClick={() => { handleShow() }}>Add New Test</Button></div>}
            ...
            {question_sets.length > 0 ? question_sets?.map(question_set => {
                return (
                    <>
                    <div className="col-xl-12 col-lg-4 mb-4 text-black">
                     <div className="card mt-2  text-primary-emphasis bg-info-subtle border-primary-subtle rounded-3  w-75 h-100 p-2">
                         <div className="card-body d-flex flex-column align-items-center">
                             ...
                             <button onClick={() => { handlenavigate(question_set._id,question_set.testname) }} className="btn btn-primary border-white mt-auto">Edit Test</button>
                         </div>
                     </div>
                    </div>
