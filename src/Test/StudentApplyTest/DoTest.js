...
function DoTest(props) {
  const [useruid, setuseruid] = useState(JSON.parse(localStorage.getItem("trackfinity-user"))._id)
  ...
  const handleStartTest = () => {
    fetch(url + "/user/test/updateteststart/" + state.testid + "/" + useruid, {
      ...
      navigate("/studdashboard/applytest", { state: { testid: state.testid,min:rminutes,hrs:rhours,duration:data.test.duration } })
